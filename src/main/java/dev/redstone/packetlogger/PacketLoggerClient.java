...
public class PacketLoggerClient implements ClientModInitializer {
	...
	private static KeyMapping loggingKeyBinding;
	private static final KeyMapping.Category CATEGORY =
		KeyMapping.Category.register(Identifier.fromNamespaceAndPath("packet-logger", "main"));
	*///?} else {
	private static KeyBinding configKeyBinding;
	private static KeyBinding loggingKeyBinding;
	//?}

	public static void setLoggingKeybind(int keyCode) {
		//? if >=26.1 {
		/*if (loggingKeyBinding != null) {
			loggingKeyBinding.setKey(InputConstants.Type.KEYSYM.getOrCreate(keyCode));
		}
		*/		//?} elif >=1.21.9 {
		/*if (loggingKeyBinding != null) {
			loggingKeyBinding.setBoundKey(InputUtil.fromKeyCode(new KeyInput(keyCode, 0, 0)));
			KeyBinding.updateKeysByCode();
		}
		*///?} else {
		if (loggingKeyBinding != null) {
			loggingKeyBinding.setBoundKey(InputUtil.fromKeyCode(keyCode, 0));
			KeyBinding.updateKeysByCode();
		}
		//?}
	}
	...
	public void onInitializeClient() {
		...
		*///?} else {
		...
		// Registriere Logging-Keybind (default: nicht gebunden)
		//? if >=26.1 {
		/*loggingKeyBinding = KeyMappingHelper.registerKeyMapping(new KeyMapping(
			"key.packet-logger.logging",
			InputConstants.Type.KEYSYM,
			GLFW.GLFW_KEY_UNKNOWN,
			CATEGORY
		));
		*///?} elif >=1.21.9 {
		/*loggingKeyBinding = KeyBindingHelper.registerKeyBinding(new KeyBinding(
				"key.packet-logger.logging",
				InputUtil.Type.KEYSYM,
				GLFW.GLFW_KEY_UNKNOWN,
				new net.minecraft.client.option.KeyBinding.Category(
		...
		*///?} else {
		...
		ClientTickEvents.END_CLIENT_TICK.register(client -> {
			...
			*///?} else {
			...
			// Logging-Keybind: Toggle logPackets
			//? if >=26.1 {
			/*while (loggingKeyBinding.consumeClick()) {
				ModConfig.getInstance().logPackets = !ModConfig.getInstance().logPackets;
			}
			*///?} else {
			while (loggingKeyBinding.wasPressed()) {
				ModConfig.getInstance().logPackets = !ModConfig.getInstance().logPackets;
			}
			//?}
		});
