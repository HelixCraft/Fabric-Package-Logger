...
/*public class ColorEditorScreen extends Screen {
    ...
    private EditBox sField, vField;
    private EditBox rField, gField, bField, aField;
    ...
    public ColorEditorScreen(Screen parent, int initialColor, Consumer<Integer> onColorSelected) {
        super(Component.literal("Color Editor"));
        this.parent = parent;
    ...
    protected void init() {
        ...
        sField = createInputField(inputX, inputY, inputWidth, inputHeight);
        sField.setResponder(s -> updateFromHSVFields());
        inputY += rowSpacing;

        vField = createInputField(inputX, inputY, inputWidth, inputHeight);
        vField.setResponder(s -> updateFromHSVFields());
        inputY += rowSpacing;

        // RGB Inputs
        rField = createInputField(inputX, inputY, inputWidth, inputHeight);
        rField.setResponder(s -> updateFromRGBFields());
        inputY += rowSpacing;

        gField = createInputField(inputX, inputY, inputWidth, inputHeight);
        gField.setResponder(s -> updateFromRGBFields());
        inputY += rowSpacing;

        bField = createInputField(inputX, inputY, inputWidth, inputHeight);
        bField.setResponder(s -> updateFromRGBFields());
        inputY += rowSpacing;

        // Alpha Input
        aField = createInputField(inputX, inputY, inputWidth, inputHeight);
        aField.setResponder(s -> updateFromAlphaField());

        // Hex Field
        int hexWidth = 80;
        hexField = new EditBox(this.font, dialogX + 50, dialogY + dialogHeight - 35, hexWidth, 18, Component.empty());
        ...
        ).bounds(dialogX + dialogWidth - 110, dialogY + dialogHeight - 35, 45, 20).build());

        this.addRenderableWidget(Button.builder(
            Component.literal("Cancel"),
            button -> this.onClose()
        ).bounds(dialogX + dialogWidth - 60, dialogY + dialogHeight - 35, 45, 20).build());

        updateFields(true); // Initial populate
    }

    private EditBox createInputField(int x, int y, int width, int height) {
    ...
    private void updateFromHSVFields() {
        ...
        try {
            float s = Float.parseFloat(sField.getValue()) / 100f;
            float v = Float.parseFloat(vField.getValue()) / 100f;
            this.saturation = Mth.clamp(s, 0, 1);
    ...
    private void updateFromRGBFields() {
        if (isUpdatingFields) return;
        try {
            int r = Integer.parseInt(rField.getValue());
            int g = Integer.parseInt(gField.getValue());
            int b = Integer.parseInt(bField.getValue());
            float[] hsv = rgbToHsv(Mth.clamp(r, 0, 255), Mth.clamp(g, 0, 255), Mth.clamp(b, 0, 255));
            this.hue = hsv[0];
    ...
    private void updateFromAlphaField() {
        if (isUpdatingFields) return;
        try {
            int a = Integer.parseInt(aField.getValue());
            this.alpha = Mth.clamp(a, 0, 255) / 255f;
            updateFields(false);
        } catch (NumberFormatException ignored) {}
    }
    ...
    private void updateFromHexField() {
        if (isUpdatingFields) return;
        String text = hexField.getValue().replace("#", "");
        try {
    ...
    private void updateFields(boolean forceAll) {
        ...
        if (forceAll || !sField.isFocused()) sField.setValue(String.valueOf((int)(saturation * 100)));
        if (forceAll || !vField.isFocused()) vField.setValue(String.valueOf((int)(value * 100)));

        if (forceAll || !rField.isFocused()) rField.setValue(String.valueOf(rgb[0]));
        if (forceAll || !gField.isFocused()) gField.setValue(String.valueOf(rgb[1]));
        if (forceAll || !bField.isFocused()) bField.setValue(String.valueOf(rgb[2]));

        if (forceAll || !aField.isFocused()) aField.setValue(String.valueOf(a));
        if (forceAll || !hexField.isFocused()) hexField.setValue(String.format("#%08X", getARGB()));
    ...
    public void extractBackground(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        // Stattdessen nur ein halbtransparentes schwarzes Overlay zeichnen:
    ...
    public void extractRenderState(GuiGraphicsExtractor context, int mouseX, int mouseY, float delta) {
        ...
        context.outline(dialogX, dialogY, dialogWidth, dialogHeight, 0xFFFFFFFF);

        context.text(this.font, "", dialogX + PADDING, dialogY + PADDING, 0xFFFFFFFF, false);
        ...
        context.outline(startX, previewY, SV_SIZE, PREVIEW_HEIGHT, 0xFF888888);
        ...
        // Hex Label
        context.text(this.font, "HEX:", dialogX + 15, dialogY + dialogHeight - 30, 0xFFAAAAAA, false);
    ...
    private void renderSVPicker(GuiGraphicsExtractor context, int x, int y) {
        ...
        context.outline(circleX - 2, circleY - 2, 5, 5, 0xFF000000); // Inner black
        context.outline(circleX - 3, circleY - 3, 7, 7, 0xFFFFFFFF); // Outer white
    }
    ...
    private void renderHueSlider(GuiGraphicsExtractor context, int x, int y) {
        ...
        handleY = Mth.clamp(handleY, y, y + SV_SIZE - 2);

        context.fill(x - 2, handleY, x + HUE_WIDTH + 2, handleY + 2, 0xFFFFFFFF);
    }

    private void renderParameterSliders(GuiGraphicsExtractor context, int x, int y, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < 6; i++) {
            int cy = y + i * 20;
            context.text(this.font, labels[i], x, cy + 2, 0xFFFFFFFF, false);
    ...
    private void drawSliderTrack(GuiGraphicsExtractor context, int x, int y, int type) {
        int w = SLIDER_WIDTH;
        int h = SLIDER_HEIGHT;
        ...
        context.outline(x - 1, y - 1, w + 2, h + 2, 0xFF444444);
    }
    ...
    private void fillCheckerboard(GuiGraphicsExtractor context, int x, int y, int w, int h) {
    ...
    private void updateSV(double mouseX, double mouseY, int x, int y) {
        this.saturation = Mth.clamp((float)(mouseX - x) / SV_SIZE, 0f, 1f);
        this.value = Mth.clamp(1.0f - (float)(mouseY - y) / SV_SIZE, 0f, 1f);
        updateFields(true);
    }

    private void updateHue(double mouseY, int y) {
        this.hue = Mth.clamp((float)(mouseY - y) / SV_SIZE, 0f, 1f) * 360f;
        updateFields(true);
    }
    ...
    private void updateSlider(int type, double mouseX, int x) {
        float t = Mth.clamp((float)(mouseX - x) / SLIDER_WIDTH, 0f, 1f);
        switch (type) {
...
/*public class ColorEditorScreen extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        fillCheckerboard(context, startX, previewY, SV_SIZE, PREVIEW_HEIGHT);
        context.fill(startX, previewY, startX + SV_SIZE, previewY + PREVIEW_HEIGHT, getARGB());
        context.drawStrokedRectangle(startX, previewY, SV_SIZE, PREVIEW_HEIGHT, 0xFF888888);
        int hueX = startX + SV_SIZE + 10;
        renderHueSlider(context, hueX, startY);
        int slidersX = hueX + HUE_WIDTH + 10;
        renderParameterSliders(context, slidersX, startY, mouseX, mouseY);
    ...
    private void renderSVPicker(DrawContext context, int x, int y) {
        ...
        int circleX = x + (int)(saturation * SV_SIZE);
        int circleY = y + (int)((1.0f - value) * SV_SIZE);
        context.drawStrokedRectangle(circleX - 2, circleY - 2, 5, 5, 0xFF000000);
        context.drawStrokedRectangle(circleX - 3, circleY - 3, 7, 7, 0xFFFFFFFF);
    }
    ...
    private void drawSliderTrack(DrawContext context, int x, int y, int type) {
        ...
        context.drawStrokedRectangle(x - 1, y - 1, w + 2, h + 2, 0xFF444444);
    }

    private int getSliderColor(int type, float t) {
        int[] rgb = hsvToRgb(hue, saturation, value);
        switch (type) {
            case 1: return hsvToRgbInt(hue, t, value);
            case 2: return hsvToRgbInt(hue, saturation, t);
            case 3: return packArgb(255, (int)(t*255), rgb[1], rgb[2]);
            case 4: return packArgb(255, rgb[0], (int)(t*255), rgb[2]);
            case 5: return packArgb(255, rgb[0], rgb[1], (int)(t*255));
            default: return 0xFFFFFFFF;
        }
    }
    ...
    public boolean mouseClicked(Click click, boolean isRelease) {
        if (super.mouseClicked(click, isRelease)) return true;
        ...
        int slidersX = hueX + HUE_WIDTH + 10 + 15;
        for (int i = 0; i < 6; i++) {
            int cy = startY + i * 20;
            if (isMouseOver(mouseX, mouseY, slidersX, cy, SLIDER_WIDTH, SLIDER_HEIGHT)) {
                int sliderType = i + 1;
    ...
    public boolean mouseDragged(Click click, double deltaX, double deltaY) {
        ...
        int startX = dialogX + PADDING;
        int startY = dialogY + 30;
        if (draggingSV) { updateSV(mouseX, mouseY, startX, startY); return true; }
        if (draggingHue) { updateHue(mouseY, startY); return true; }
        int slidersX = startX + SV_SIZE + 10 + HUE_WIDTH + 10 + 15;
        for (int i = 0; i < 6; i++) {
            int sliderType = i + 1;
            if (isDragging(sliderType)) { updateSlider(sliderType, mouseX, slidersX); return true; }
        }
    ...
    private void setDragging(int i, boolean val) {
        switch(i) {
            case 0: draggingH = val; break; case 1: draggingS = val; break;
            case 2: draggingV = val; break; case 3: draggingR = val; break;
            case 4: draggingG = val; break; case 5: draggingB = val; break;
            case 6: draggingA = val; break;
        }
    }
    ...
    private boolean isDragging(int i) {
        switch(i) {
            case 0: return draggingH; case 1: return draggingS;
            case 2: return draggingV; case 3: return draggingR;
            case 4: return draggingG; case 5: return draggingB;
            case 6: return draggingA; default: return false;
        }
    }
    ...
    private void updateSlider(int type, double mouseX, int x) {
        ...
        switch (type) {
            ...
            case 3: case 4: case 5: {
                ...
                rgb[type - 3] = (int)(t * 255);
                float[] hsv = rgbToHsv(rgb[0], rgb[1], rgb[2]);
                hue = hsv[0]; saturation = hsv[1]; value = hsv[2]; break;
            }
            case 6: alpha = t; break;
        }
        updateFields(true);
    ...
    private void setColorFromARGB(int argb) {
        this.alpha = ((argb >> 24) & 0xFF) / 255.0f;
        int r = (argb >> 16) & 0xFF; int g = (argb >> 8) & 0xFF; int b = argb & 0xFF;
        float[] hsv = rgbToHsv(r, g, b);
    ...
    private static int packArgb(int a, int r, int g, int b) { return (a << 24) | (r << 16) | (g << 8) | b; }
    private static int hsvToRgbInt(float h, float s, float v) { int[] rgb = hsvToRgb(h, s, v); return packArgb(255, rgb[0], rgb[1], rgb[2]); }

    private static int[] hsvToRgb(float h, float s, float v) {
        float c = v * s; float x = c * (1 - Math.abs(((h / 60.0f) % 2) - 1)); float m = v - c;
        float r=0, g=0, b=0;
...
public class ColorEditorScreen extends Screen {
    ...
    private void renderSVPicker(DrawContext context, int x, int y) {
        ...
        int circleX = x + (int)(saturation * SV_SIZE);
        int circleY = y + (int)((1.0f - value) * SV_SIZE);
        context.drawBorder(circleX - 2, circleY - 2, 5, 5, 0xFF000000);
        context.drawBorder(circleX - 3, circleY - 3, 7, 7, 0xFFFFFFFF);
    }
